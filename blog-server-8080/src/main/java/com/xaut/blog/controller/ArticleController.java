...
public class ArticleController {
    ...
    public CommonResult<?> getById(HttpServletRequest request, @PathVariable("id") Long articleId) {
        ...
        if (article != null) {
            ...
            article = pageView(article, ip+"-"+articleId);
    ...
    public CommonResult<?> getByOwnerUserId(@PathVariable("id") Long ownerUserId){
        List<Article> list = articleService.getByOwnerUserId(ownerUserId);
        if (list != null) {
            return new CommonResult<List<Article>>(200, "查询成功", list);
        }
        return new CommonResult<List<Article>>(400, "查询失败");
     ...
     * @return
     */
    @GetMapping("/getbyadmin/{admin}")
    public CommonResult<?> getByUserAdmin(@PathVariable("admin") String admin){
        List<Article> list = articleService.getByUserAdmin(admin);
        if (list != null) {
            return new CommonResult<List<Article>>(200, "查询成功", list);
        }
        return new CommonResult<List<Article>>(400, "查询失败");
